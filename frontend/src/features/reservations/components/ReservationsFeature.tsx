...
export const ReservationsFeature: FC = () => {
  ...
  const [passJwtToken, setPassJwtToken] = useState<string>('');
  const [detailsJwtToken, setDetailsJwtToken] = useState<string>('');
  ...
  const handleOpenDetails = async (res: ReservationItem) => {
    setSelectedReservation(res);
    setDetailsJwtToken('');
    try {
      const resp = await client.get(`/qr/generate?entityType=RESERVATION&entityId=${res.id}`);
      setDetailsJwtToken(resp.data.qrToken);
    } catch {
      setDetailsJwtToken('');
    }
  };
        ...
        {selectedReservation && (
          <div className="space-y-4">
            ...
            <div className="p-3 bg-neutral-secondary-bg rounded-xl border border-neutral-border space-y-1.5 text-xs">
              ...
              <div className="font-mono text-[11px] text-neutral-primary select-all break-all block bg-white p-2.5 rounded-lg border border-neutral-border max-h-24 overflow-y-auto leading-relaxed">
                {detailsJwtToken || 'Loading JWT Pass Token...'}
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-1.5 text-left max-w-sm mx-auto">
                <div className="flex justify-between items-center text-xs">
                  <span className="font-bold text-neutral-secondary">JWT Gate Pass Token</span>
                  {passJwtToken && (
                    <button
                      onClick={() => {
