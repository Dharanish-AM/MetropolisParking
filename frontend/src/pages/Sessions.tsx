...
export const Sessions: FC = () => {
  ...
  const handleEndSession = (plateNumber: string) => {
    if (!window.confirm(`Are you sure you want to end the session for vehicle ${plateNumber}?`)) return;
  ...
  const filteredSessions = (sessions || []).filter((session) => {
    const matchesPlate = (session.plateNumber || "").toLowerCase().includes(searchTerm.toLowerCase()) ||
      (session.spaceNumber || "").toLowerCase().includes(searchTerm.toLowerCase());
    
    const isSessionActive = session.exitTime === null || session.exitTime === undefined || session.status === "ACTIVE";
  ...
  return (
    <div className="min-h-screen bg-neutral-bg text-neutral-primary flex flex-col font-sans">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 animate-fade-in">
          ...
          <Table>
            ...
            <TableBody>
              {isLoading ? (
                ...
                <TableRow>
                  <TableCell colSpan={8} className="text-center text-neutral-secondary font-medium py-12">
                ...
                filteredSessions.map((session) => {
                  const isActive = session.exitTime === null || session.exitTime === undefined || session.status === "ACTIVE";
                  return (
                    <TableRow key={session.id}>
                      ...
                      <TableCell className="font-bold text-neutral-primary">
                        {session.fee !== null && session.fee !== undefined ? `$${session.fee.toFixed(2)}` : "—"}
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell className="text-right">
                        {isActive ? (
                          ...
                          <span className="text-xs text-neutral-secondary font-semibold pr-3">Completed</span>
                        )}
                      </TableCell>
                    </TableRow>
                  );
        ...
        <Modal isOpen={isStartOpen} onClose={() => setIsStartOpen(false)} title="Start Parking Session">
          <form onSubmit={handleSubmit(onStartSubmit)} className="space-y-4">
            ...
            {availableSpaces.length === 0 && (
              <p className="text-xs text-status-occupied font-semibold flex items-center gap-1">
                <AlertCircle className="w-4 h-4 text-red-500" /> No available parking spots left. Make a space available first.
              </p>
            )}

            <div className="flex gap-3 justify-end pt-2">
              <Button type="button" variant="secondary" onClick={() => setIsStartOpen(false)} className="w-auto px-5">
                Cancel
              </Button>
              <Button type="submit" isLoading={startSessionMutation.status === "pending"} className="w-auto px-5" disabled={availableSpaces.length === 0}>
                Start
              </Button>
            </div>
          </form>
