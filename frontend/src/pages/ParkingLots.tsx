...
import { useLots, useCreateLot, useDeleteLot, useLevels, useCreateLevel } from "../features/lots/hooks";
import { useSpaces, useCreateSpace, useUpdateSpaceStatus, useDeleteSpace } from "../features/spaces/hooks";
import { 
  Building2, 
  Layers, 
  CheckCircle2, 
...
export const ParkingLots: FC = () => {
  ...
  const activeLotId = selectedLotId || (lots && (lots as ParkingLot[]).length > 0 ? (lots as ParkingLot[])[0].id : "");
  ...
  const filteredSpaces = (spaces as ParkingSpace[])?.filter((space) => {
  ...
  const handleAddLotSubmit = (data: LotFormValues) => {
    createLotMutation.mutate({
      ...
      onError: (err: any) => {
        setNotification({ message: err.response?.data?.message || "Failed to create lot", type: "error" });
      }
    });
  };
  ...
  const handleAddLevelSubmit = (data: LevelFormValues) => {
    createLevelMutation.mutate(data.levelNumber, {
      ...
      onError: (err: any) => {
        setNotification({ message: err.response?.data?.message || "Failed to create level", type: "error" });
      }
    });
  };
  ...
  const handleAddSpaceSubmit = (data: SpaceFormValues) => {
    ...
    createSpaceMutation.mutate({
      ...
      onError: (err: any) => {
        setNotification({ message: err.response?.data?.message || "Failed to create space", type: "error" });
      }
    });
  };
  ...
  return (
    <div className="min-h-screen bg-neutral-bg text-neutral-primary flex flex-col">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
            ...
            {activeLotId && (
              <>
                ...
                <Button
                  ...
                  size="sm"
                  onClick={() => {
                    if (confirm("Are you sure you want to delete this parking lot and all associated levels/spaces?")) {
                      deleteLotMutation.mutate(activeLotId, {
                        onSuccess: () => {
                          setNotification({ message: "Parking lot deleted successfully", type: "success" });
                          setSelectedLotId("");
                        },
                        onError: (err: any) => {
                          setNotification({ message: err.response?.data?.message || "Failed to delete lot", type: "error" });
                        }
                      });
                    }
                  }}
        ...
        <Modal isOpen={isAddLotOpen} onClose={() => setIsAddLotOpen(false)} title="Create New Parking Lot">
          <LotForm onSubmit={handleAddLotSubmit} isLoading={createLotMutation.status === "pending"} />
        </Modal>

        <Modal isOpen={isAddLevelOpen} onClose={() => setIsAddLevelOpen(false)} title="Add Parking Level">
          <form onSubmit={handleLevelSubmit(handleAddLevelSubmit)} className="space-y-4">
            ...
            />
            <Button type="submit" isLoading={createLevelMutation.status === "pending"} className="w-full">
              Add Level
            </Button>
          </form>
        </Modal>

        <Modal isOpen={isAddSpaceOpen} onClose={() => setIsAddSpaceOpen(false)} title="Add Parking Space">
          <SpaceForm onSubmit={handleAddSpaceSubmit} isLoading={createSpaceMutation.status === "pending"} />
        </Modal>
                  ...
                  onClick={() => {
                    if (confirm("Are you sure you want to delete this parking space?")) {
                      deleteSpaceMutation.mutate(selectedSpace.id, {
                        onSuccess: () => {
                          setNotification({ message: "Space deleted successfully", type: "success" });
                          setSelectedSpace(null);
                        },
                        onError: (err: any) => {
                          setNotification({ message: err.response?.data?.message || "Failed to delete space", type: "error" });
                        }
                      });
                    }
                  }}
